    ...
    def get_hyper_index(self, index: int = None, page_size: int = 10) -> Dict:
        """
            ...
            'index': the current start index of the return page. That is, the
                index of the first item in the current page. For example,
                if requesting page 3 with page_size 20, and no data was removed
                from the dataset, the current index should be 60.
            'next_index': the next index to query with. That should be the
                index of the first item after the last on the current page.
        ...
        while len(data) < page_size:
            ...
            next_index += 1  # Skip deleted items
